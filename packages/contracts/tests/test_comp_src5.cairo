...
use orbis::introspection::interface::ISRC5;
use orbis::introspection::interface::ISRC5Legacy;
use orbis::introspection::interface::{SRC5_INTERFACE_ID, SRC5_INTERFACE_ID_OLD};
use orbis::introspection::src5::src5_component;
use orbis::mocks::src5_mocks::SRC5Mock;
...
fn test_introspection_outside_execution_id() {
    let mut component = COMPONENT_STATE();
    assert!(
        component.supports_interface(ERC165_OUTSIDE_EXECUTION_INTERFACE_ID_REV_0), "should support",
    );
    assert!(
        component.supports_interface(ERC165_OUTSIDE_EXECUTION_INTERFACE_ID_REV_1), "should support",
    );
}
...
fn test_introspection_legacy_method() {
    ...
    assert_eq!(
        component.supportsInterface(ERC165_OUTSIDE_EXECUTION_INTERFACE_ID_REV_0),
        1,
        "should support",
    );
    assert_eq!(
        component.supportsInterface(ERC165_OUTSIDE_EXECUTION_INTERFACE_ID_REV_1),
        1,
        "should support",
    );
